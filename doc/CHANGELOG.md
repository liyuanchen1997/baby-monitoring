...
## 2026-08-15 · 拍摄端布局优化 · v1.0.4

- **更新**：拍摄端布局重构——原房间码+二维码大卡片改为紧凑房间码条（头部下方单行：房间码 + 观看端状态 + 「📱 二维码」按钮）；二维码收纳进弹层（点击展示、遮罩/关闭按钮关闭），小屏拍摄界面释放大量空间
- **更新**：设计文档 §11 补充拍摄端 UI 说明（原"QrOverlay 刷新按钮"描述未实现，已修正为弹层交互）
- **验证**：e2e 9/9 通过（二维码断言改为弹层流程：点击按钮 → 等 render 完成 → 校验 URL）

## 2026-08-15 · 拍摄端 UI 细节 · v1.0.5

- **更新**：房间码条左侧增加「房间码」提示标签，房间码字号调整至 30px；二维码弹层内"扫码进入观看端"文字居中（qr-box 改为 flex 纵向居中）
- **验证**：e2e 9/9 通过
...
## 2026-08-15 · 开发记忆文档 · v1.1.1

- **新增**：doc/开发记忆.md — 项目状态/架构速览/今日成果/遗留事项（低优先级效率项 + 待真机验证项 + 产品增强候选）/环境命令/工作流约定，供日后继续开发快速恢复上下文
